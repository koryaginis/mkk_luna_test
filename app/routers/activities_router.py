...
from ..schemas import ActivitySchema, ActivityBaseSchema, ActivityUpdateSchema
...
@router.get("/", response_model=List[ActivitySchema], status_code=status.HTTP_200_OK)
async def get_activities_list_endpoint(
    db: AsyncSession = Depends(get_db),
    _: None = Depends(verify_api_key),
):
    """
    Эндпоинт для получения всех видов деятельности.
    """
    return await get_activity_tree(db=db, full_tree=True)
